...
class Car: 
    def __init__(self, speed = 3, size = 20, color = (255, 100, 100)):
        self.x = WIDTH / 2
        self.y = 0
        self.speed_x = 0
        self.speed_y = 0
...
class DrivingGame:
    def __init__(self, race_length = 3, dificaulty = 3, road_width = 200):
    ...
    def generate_race(self):
        ...
        for i in range(1, self.race_length * self.dificaulty + 1):
            ...
            self.race_path.append(
                (
                    randint(self.road_width // 2, WIDTH - self.road_width // 2),
                    this_height
                )
            )
    ...
    def calculate_frame(self):
        ...
        self.camera_y = total_y / len(self.cars) - WIDTH / 2
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a: direction_R = True
            if event.key == pygame.K_d: direction_L = True
        
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_a: direction_R = False
            if event.key == pygame.K_d: direction_L = False


    if direction_R: cars[0].angle += math.pi / 60
    if direction_L: cars[0].angle -= math.pi / 60
